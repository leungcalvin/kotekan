...
basebandDumpData::Status basebandReadout::extract_data(basebandDumpData data) {
    ...
    // CL: TODO: GET GAINS HERE. JUST READ IN YOUR FREQUENCIES
    timeval tmp, delta;
    delta.tv_sec = 0;
    ...
    // CL: TODO: CHANGE TO THE FOLLOWING
    // for (int frame_index = data.dump_start_frame - 1000; !stop_thread && frame_index < data.dump_end_frame + 1000;
	...
	// CL: TODO: INITIALIZE GAIN * STEERING PHASE ARRAY GtimesSP of shape num_local_freq * num_pointing * num_elements
	// CL: TODO: INITIALIZE BF_BUFFER_OUT of shape num_local_freq * num_pointing * _samples_per_data_set
		...
		// CL: TODO: RUN PHASE GENERATION HERE ONCE PER BUFFER FRAME. JUST DO YOUR FREQUENCIES
		// CL: TODO: ONCE PER BUFFER FRAME, RECALCULATE GtimesSP = GAIN * STEERING PHASE [ARRAY OF SIZE _num_local_freq * _num_elements]
            }
		...
		// CL: we are going to try to implement an operation equivalent to the following: memcpy to BF_BUFFER_IN; beamform, memcpy to BF_BUFFER_OUT:
                // memcpy(out_frame + (out_start * _num_elements),
                //       *BF_BUFFER_IN, copy_len * _num_elements)
                // memcpy(*BF_BUFFER_IN, copy_len * _num_elements) 
                //       in_buf_data + (in_start * _num_elements), copy_len * _num_elements);
		// CL: BUT ACTUALLY, we will just try to do it in one go to keep us at one memcpy by writing a drop-in replacement for memcpy.
		// USE AI TO WRITE AND TEST A DROP IN REPLACEMENT....
		// NOTE THAT WE HAVE AVX512 VECTOR INSTRUCTIONS ENABLED
		// OUTPUT array should be complex64 of size copy_len *
            } else {
		...
		// BEAMFORM DO NOT MEMCPY INTO THE OUTPUT FRAME
		// need something like:
                memcpy(out_frame + (out_start * _num_elements),
                       in_buf_data
                           + (in_start * _num_freq_per_stream + stream_freq_idx) * _num_elements,
